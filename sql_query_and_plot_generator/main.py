...
def main():
    """
    Launches the Streamlit UI for SQL Query and Plot Generator.

    This function initializes the web interface where users can input 
    natural language requests, which will be converted into SQL queries 
    ...
    if "hide_save_button" not in st.session_state:
        # Empêche d'afficher "Save Query" après "Don't Save"
        st.session_state.hide_save_button = False

    # Streamlit UI
    st.set_page_config(page_title="SQL Query Generator", layout="wide")

    # Add Scibids logo at the top
    st.image(r"Images\DV-SCIBIDS-650.png", width=250)

    st.title("🛠️ SQL Query Generator")
    st.write(
        "Enter a natural language request, "
        "and the system will generate the corresponding SQL query."
    )
    ...
    with col1:
        ...
        selected_table = st.selectbox(
            "Select a table to view its schema:", options=list(DB_SCHEMAS.keys()))
        ...
        table_name = schema_lines[0].replace("Table: ", "").strip(
        ) if "Table: " in schema_lines[0] else selected_table
        ...
        for line in schema_lines:
            if "- " in line and ":" in line:
                ...
                column_data.append(
                    {"Column Name": column_name, "Description": column_desc})
        ...
        with st.expander(f"🔍 **View Schema for `{table_name}`**", expanded=True):
            st.markdown(
                f"### 📝 Table Name: `{table_name}`", unsafe_allow_html=True)
    ...
    with col2:
        ...
        user_input = st.text_area(
            "Describe what you need:", placeholder="Example: Quels sont les ab tests?")

        if st.button("Get Answer"):
            try:
                response = intelligent_agent(
                    user_input)  # Returns multiple tasks
                                ...
                                with col1:
                                    if st.button("💾 Save Query", key="save_button"):
                                        ...
                                        if "query" in saved_queries.columns:
                                            is_duplicate = saved_queries["query"].astype(
                                                str).str.strip().eq(user_input.strip()).any()
                                        else:
                                            is_duplicate = False

                                        # Debugging
                                        print(
                                            f"🔎 Est-ce un doublon ? {is_duplicate}")

                                        if not is_duplicate:
                                            print(
                                                "✅ Nouvelle requête détectée, enregistrement en cours...")
                                            new_entry = pd.DataFrame(
                                                [{"query": user_input, "reference_sql": st.session_state.cleaned_sql_query}])
                                            updated_queries = pd.concat(
                                                [saved_queries, new_entry], ignore_index=True)
                                            updated_queries.to_csv(
                                            ...
                                            # Debugging
                                            print("⚠️ Requête déjà enregistrée")
                                            st.warning(
                                                "⚠️ This query is already saved.")

                                with col2:
                                    if st.button("❌ Don't Save", key="dont_save_button"):
                                        # Debugging
                                        print("❌ Bouton Don't Save cliqué")
