"""
Utility functions for SQL query generation, execution, and visualization.

This module provides:
- SQL query cleaning and validation.
- BigQuery query execution.
- Intelligent table recognition based on user queries.
- SQL query generation.
- Dynamic visualization generation.
- AI-powered analysis on query results.
...
def intelligent_agent(user_prompt):
    ...
    **Example Breakdown:**
    **User Query:** *"Get the revenue per region"*
    **Decomposed Tasks:**
    [
        {{"task": "sql", "content": "Generate an SQL query to compute the total revenue per region from the sales data."}},
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are an expert in SQL database structures. "
                "Given a user request, identify the most relevant tables "
                "from the schema provided that will be called in the SQL query."
            ),
        },
        {
            "role": "user",
...
def generate_sql(user_query, selected_tables):
    ...
    keys_identification = {
    "io_benchmark": [
        "group_object_field_id", "dsp", "kpi"
    ],
    "new_benchmark": [
        "kpi", "dsp"
    ],
    "t_daily_io_features": [
        "dsp", "group_object_field_id", "day_utc"
    ],
    "t_flights": [
        "group_object_field_id", "dsp", "object_field_id", "gof_flight_id", "of_flight_id"
    ],
    "t_insertion_orders": [
        "group_object_field_id", "dsp"
    ],
    "t_campaign_performance_day": [
        "group_object_field_id", "dsp", "object_field_id", "gof_flight_id", "date_hour_tz"
    ],
    "t_reach_performance": [
        "group_object_field_id", "dsp", "end_date"
    ],
    "t_pixel_performance": [
        "group_object_field_id", "object_field_id", "dsp", "conversion_pixel_id", "date_hour_tz"
    ],
}

    # Extract only the relevant keys for the selected tables
    relevant_keys = "\n".join(
        [f"{table}: {', '.join(keys_identification[table])}" for table in selected_tables if table in keys_identification])
    relevant_schemas = "\n\n".join([DB_SCHEMAS[table.strip(
    )] for table in selected_tables if table.strip() in DB_SCHEMAS])
...
def generate_visualization_code(user_prompt, results):
    ...
    categorical_cols = results.select_dtypes(
    ...
    system_prompt = f"""
    You are a Python visualization expert that generates **Matplotlib/Seaborn plots for Streamlit**.
...
def visualize_data(visualization_code, results):
    ...
    try:
        ...
        st.error(
            f"❌ An error occurred while executing visualization: {str(e)}")
